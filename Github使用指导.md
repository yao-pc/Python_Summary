# github设定标题
> 在 GitHub 的 Markdown 文件中，设定标题非常简单，只需在行首使用 # 符号（井号）即可。# 的数量代表标题的层级，数量越少，标题越大（层级越高）。

GitHub 原生支持 1 级到 6 级 标题：

📝 语法速查表
层级	Markdown 语法	渲染效果（模拟）	适用场景
一级标题	# 标题内容	一级标题	文档主标题（README 通常只用一个）
二级标题	## 标题内容	二级标题	主要章节（如“项目介绍”、“使用方法”）
三级标题	### 标题内容	三级标题	章节下的子模块
四级标题	#### 标题内容	四级标题	更细致的分类
五级标题	##### 标题内容	五级标题	较少使用，用于细分
六级标题	###### 标题内容	六级标题	极少使用，用于极细分
⚠️标题后必须加一个空格，否则语法可能失效！
